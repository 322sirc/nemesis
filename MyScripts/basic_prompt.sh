...
read -rp "📦 Enter the ROOT partition (e.g. /dev/sda2 or /dev/nvme0n1p5): " ROOT_PART
read -rp "💾 Enter the EFI partition (e.g. /dev/sda1 or /dev/nvme0n1p1): " EFI_PART
MOUNT_ROOT="/mnt"

# Check that both partitions exist
[[ -b "$ROOT_PART" ]] || { echo "❌ Root partition $ROOT_PART does not exist!"; exit 1; }
[[ -b "$EFI_PART" ]] || { echo "❌ EFI partition $EFI_PART does not exist!"; exit 1; }
...
pacstrap -K "$MOUNT_ROOT" base reflector linux linux-headers sudo grub efibootmgr nano iwd networkmanager --noconfirm
